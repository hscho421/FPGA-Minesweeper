...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int ret = XSpi_Transfer	(&SpiInstance,input,recv,2);
	if(ret)
		xil_printf("reg_wr error\n");
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	u8 input[nbytes+1], recv[nbytes+1];
	input[0] = (u8) (reg+2);
	for (int i = 0; i < nbytes; i++) {
		input[i+1] = (u8)data[i];
	}
	XSpi_SetSlaveSelect	(&SpiInstance,1);
	int ret = XSpi_Transfer	(&SpiInstance,input,recv,nbytes+1);
	if(ret)
		xil_printf("bytes_wr error\n");
	XSpi_SetSlaveSelect	(&SpiInstance,0);
	return data + nbytes;
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int ret = XSpi_Transfer	(&SpiInstance,input,recv,nbytes+1);
	if(ret)
		xil_printf("bytes_rd error\n");
	XSpi_SetSlaveSelect	(&SpiInstance,0);
	for(int i = 0; i < nbytes; i++){
		data[i] = (BYTE)recv[i+1];
	}
	return data + nbytes;
}
