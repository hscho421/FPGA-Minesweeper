    ...
    while (1) {
        ...
        if (GetUsbTaskState() == USB_STATE_RUNNING) {
            if (!runningdebugflag) {
                ...
                // Output first two keycodes to hex displays
                u32 keycode_data = 0;
                keycode_data = kbdbuf.keycode[0]; // First keycode to first display
                printHex(keycode_data, 1);

                keycode_data = kbdbuf.keycode[1]; // Second keycode to second display
                printHex(keycode_data, 2);
            }
            else if (device == 2) {
                rcode = mousePoll(&buf);
                if (rcode == hrNAK) {
